...
## 5. Запуск початкового прикладу (Завдання 3)
...
Наданий викладачем код розміщено в підпакеті `fpv_lab2/flight_test/`
відповідно до його структури, виконуваний вузол додано до секції
`console_scripts` у `setup.py`:
...
Вивід останньої команди:

```
fpv_lab2 flight_test
fpv_lab2 goto_point
```

### Параметр use_sim_time

Приклад запускається командою:

```bash
ros2 run fpv_lab2 flight_test --ros-args -p use_sim_time:=true
```

Параметр обов'язковий. Без нього приклад **стабільно падає** з помилкою
`Drone disarmed unexpectedly during flight` одразу після озброєння двигунів.

Причина: цикл керування вузла працює з періодом 0,2 с за годинником
реального часу, а ArduPilot публікує тему `/ap/v1/status` раз на 100 мс
**симуляційного** часу (константа `AP_DDS_DELAY_STATUS_TOPIC_MS`). При
коефіцієнті реального часу близько 0,25 це відповідає приблизно 400 мс
реального часу. Обробник стану зависання перевіряє поле `armed` уже через
0,2 с після озброєння і читає ще не оновлене значення `false`, після чого
зупиняє програму. Переведення вузла на симуляційний час ставить цикл
керування і тему статусу на один годинник. Наданий викладачем код при цьому
не змінювався.

### Журнал виконання

```
Flight test node started
Waiting for ArduPilot and ROS services...
ArduPilot and all services are available
Running pre-arm check
Pre-arm check passed: Vehicle is Armable
Requesting flight mode 4
GUIDED mode enabled
Requesting motor arming
Motors armed
Requesting takeoff to 2.0 m
Takeoff command accepted
Hovering for 30.0 seconds
Hover completed, starting landing
Requesting flight mode 9
LAND mode enabled
Landing completed
...
Повний журнал — у каталозі `logs/`.

### Перевірка утримання висоти в режимі зависання

Пункт 12 завдання вимагає перевірити утримання висоти. Для цього написано
допоміжний вузол `scripts/record_altitude.py`, який паралельно з польотом
пише висоту з теми `/odometry` у CSV з абсолютними мітками часу, що дозволяє
точно накласти запис на журнал польоту:

```bash
python3 ~/fpv_labs/scripts/record_altitude.py ~/fpv_labs/logs/hover_altitude.csv 5
```

Результат запису з частотою 5 Гц (`logs/hover_altitude.csv`), відлік від
команди зльоту:

| Фаза | Тривалість | Опис |
|---|---|---|
| 0,0 – 18,0 с | 18,0 с | апарат на землі, розкрутка двигунів |
| 18,0 – 23,8 с | 5,8 с | набір висоти |
| 23,8 – 33,3 с | 9,5 с | зависання на висоті |

Статистика за 33 заміри у фазі зависання:

| Показник | Значення |
|---|---|
| Мінімальна висота | 2,157 м |
| Максимальна висота | 2,234 м |
| Середня висота | 2,197 м |
| Розкид | 0,077 м |
| Середньоквадратичне відхилення | 0,0174 м |
| **Висота над точкою старту** | **2,00 м** (задано 2,0 м) |

Координата `z` теми `/odometry` відлічується від початку координат світу, а
не від апарата: у дрона, що стоїть на землі, вона дорівнює 0,195 м — це
висота початку системи координат корпусу над ґрунтом. Тому фактична висота
зависання становить 2,197 − 0,195 = 2,00 м, що точно відповідає заданій.

Утримання висоти підтверджено: відхилення не перевищує 8 см при
середньоквадратичному відхиленні 1,7 см.

### Зауваження до тривалості зависання

Константа `HOVER_DURATION = 30.0` у наданому прикладі відлічується від
моменту прийняття команди зльоту, а не від досягнення висоти. Через це з
30 секунд на власне зависання припадає лише близько 9,5 секунди, решта —
розкрутка двигунів і набір висоти. У власному модулі `goto_point` ця
особливість врахована: відлік стабілізації починається лише після
фактичного досягнення заданої висоти.

## 6. Алгоритм польоту до заданої точки (Завдання 4)
...
| `axis_direction` | 1.0 | напрямок осі зміщення (див. п. 7) |
...
## 7. Напрямок зміщення цілі — розбіжність із Додатком 2
...
## 8. Проблеми, виявлені під час розгортання, та їх усунення
